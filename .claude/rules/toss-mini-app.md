...
**Read `.ai-factory/apps-in-toss-essential.txt` before using any `@apps-in-toss/web-framework` API.** It contains verified exports. If an API is not listed there, it does not exist — do not guess.
...
- Read `.ai-factory/tds-reference.txt` first — official TDS LLM doc
