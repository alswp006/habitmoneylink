## Task: S5 주간 리포트 /report (요약 화면, 게이트는 라우팅에서 적용)
주간 리포트 페이지를 MVP 요약으로 구현한다. 현재 주(weekStartsOn 설정 기반)의 체크인 합계(savedAmountKRW)와 습관별 합계를 간단히 보여준다. 보상형 광고 게이트(TossRewardAd)는 마지막 통합(App.tsx)에서 라우트 요소를 감싸 적용한다.
...
Create `src/__tests__/packet-0012.test.ts` with tests for the Acceptance Criteria above.
...
- Commit: `git add src/__tests__/packet-0012.test.ts && git commit -m "test: TDD red phase — packet 0012"`

### Step 2: Verify Red (tests should FAIL)
```bash
pnpm test src/__tests__/packet-0012.test.ts
```
...
- Commit: `git add <source files> && git commit -m "feat: implement packet 0012"`
