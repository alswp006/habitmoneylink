## Task: localStorage base IO 유틸 + 키 상수
localStorage 접근을 단일 유틸로 통일하기 위해 키 상수와 readRaw/safeJsonParse/safeSetItem를 구현한다. JSON.parse 실패는 PARSE_ERROR(500), 저장공간 초과는 QUOTA_EXCEEDED(507)로 표준화해 상위 레이어에서 동일한 방식으로 처리할 수 있게 한다.
...
src/lib/storage/keys.ts, src/lib/storage/base.ts
