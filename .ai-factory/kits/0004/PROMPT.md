## Task: KST 날짜 유틸 + AppStore 하이드레이션/인덱스/FK검증/리셋 + write 뮤테이션
KST 기준 오늘(YYYY-MM-DD) 생성 유틸을 제공하고, AppStoreProvider에서 5개 엔티티를 hydrate하며 Map/Set 인덱스를 만든다. checkIns.habitId FK를 검증해 실패 시 SCHEMA_MISMATCH(500) 상태로 전환한다. 또한 create/update/delete 및 체크인/목표/배지 뮤테이션을 구현하고 localStorage에 반영한다.
...
src/lib/date/kst.ts, src/lib/store/AppStore.tsx
...
- Packet 0003: "엔티티 get/set + 최소 스키마 검증(타입가드)" (files: src/lib/storage/validate.ts, src/lib/storage/entities.ts)
