## Task: 공용 UI 컴포넌트: 습관 행(HabitListRow)
홈의 활성 습관 리스트 한 줄 UI를 컴포넌트로 분리한다. ListRow 우측에 ‘오늘 참았어요’ 버튼을 배치하고, 행 탭과 버튼 탭을 분리해 콜백을 각각 호출한다. 표시 문구는 부모에서 전달받는다.
...
src/components/HabitListRow.tsx
...
## Page Type: List (TDS 패턴)
참조: tds-patterns.md의 "ListRow stack + onClick". 빈 상태 + 로딩 상태 처리 필수.
```tsx
<>
  <Top title={<Top.TitleParagraph>목록</Top.TitleParagraph>} />
  {loading && <div style={{ padding: 16 }}><Skeleton /></div>}
  {!loading && items.length === 0 && (
    <div style={{ padding: 32, textAlign: 'center' }}>
      <Paragraph.Text typography="t5" color="secondary">아직 항목이 없어요</Paragraph.Text>
    </div>
  )}
  {items.map((item) => (
    <ListRow
...
- Packet 0001: "논리 스키마 Type 고정 + StorageError/HTTP-like status + RouteState" (files: src/lib/types.ts)
