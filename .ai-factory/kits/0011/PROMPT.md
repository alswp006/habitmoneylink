## Task: S4 목표 설정 /goal (활성 목표 upsert)
목표 설정 화면을 MVP 수준으로 구현한다. 기존 activeGoal이 있으면 입력 필드에 채워 보여주고, 저장 시 upsertActiveGoal({title,targetAmountKRW})를 호출한다. 숫자 입력은 정수만 허용하고 범위(1,000..100,000,000)는 저장 시 검증한다.
