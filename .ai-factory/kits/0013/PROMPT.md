## Task: S6 배지 보관함 /badge (배지 리스트)
배지 보관함을 MVP로 구현한다. badges 배열을 ListRow로 렌더링하고, 'streak_7' 배지의 unlockedAt 유무에 따라 '획득함/잠김' 상태 문구를 표시한다. 잠긴 경우 '배지 잠금 해제'로 이동할 수 있는 진입 ListRow를 제공한다.
...
Create `src/__tests__/packet-0013.test.ts` with tests for the Acceptance Criteria above.
...
- Commit: `git add src/__tests__/packet-0013.test.ts && git commit -m "test: TDD red phase — packet 0013"`

### Step 2: Verify Red (tests should FAIL)
```bash
pnpm test src/__tests__/packet-0013.test.ts
```
...
- Commit: `git add <source files> && git commit -m "feat: implement packet 0013"`
