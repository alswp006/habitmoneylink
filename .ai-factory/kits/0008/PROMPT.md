## Task: S1 홈 / (로딩/빈상태/에러다이얼로그/체크인/요약/배너)
홈에서 활성 습관 목록과 오늘 체크인 액션을 제공하고, 누적 절약액(sum(savedAmountKRW))과 목표 요약을 보여준다. 스키마 손상(PARSE_ERROR/SCHEMA_MISMATCH) 시 AlertDialog로 초기화 유도, 체크인 충돌(409) 시 별도 AlertDialog를 노출한다. 배너 광고는 요약 섹션 하단에 1개 고정 배치한다.
...
- Packet 0005: "공용 UI 컴포넌트: 요약 카드(HeroSummaryCard)" (files: src/components/HeroSummaryCard.tsx)
- Packet 0006: "공용 UI 컴포넌트: 습관 행(HabitListRow)" (files: src/components/HabitListRow.tsx)
