당신은 패킷 0014 ("S7 배지 잠금 해제 /badge/unlock (언락 요청, 중복 409 처리)")의 코드 리뷰를 수행합니다.
...
## 테스트 파일
src/__tests__/packet-0014.test.ts
...
- .ai-factory/spec.md
- src/pages/BadgeUnlock.tsx
- src/__tests__/packet-0014.test.ts (존재하는 경우)
...
### Agent 2: 조용한 실패 탐지기
각 파일을 읽으세요: src/pages/BadgeUnlock.tsx
...
```
════════════════════════════════════════════════
🔍 코드 리뷰 리포트: 0014 — S7 배지 잠금 해제 /badge/unlock (언락 요청, 중복 409 처리)
════════════════════════════════════════════════
