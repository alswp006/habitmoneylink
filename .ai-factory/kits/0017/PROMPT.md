## Task: 라우팅 와이어링 + Provider 연결 + 통합 폴리시
App.tsx에 모든 페이지(Blocking.tsx, Home.tsx, HabitNew.tsx, HabitEdit.tsx, Goal.tsx, Report.tsx, Badge.tsx, BadgeUnlock.tsx) Route 연결, main.tsx 수정 금지(@AI:ANCHOR), 광고/결제 연동, 최종 UX 폴리시
...
- Packet 0016: "Integration 2/2: 광고/보상형 게이트/TabBar/507 토스트 폴리시 최종 폴리시" (files: src/App.tsx)
...
Create `src/__tests__/packet-0017.test.ts` with tests for the Acceptance Criteria above.
...
- Commit: `git add src/__tests__/packet-0017.test.ts && git commit -m "test: TDD red phase — packet 0017"`

### Step 2: Verify Red (tests should FAIL)
```bash
pnpm test src/__tests__/packet-0017.test.ts
```
...
- Commit: `git add <source files> && git commit -m "feat: implement packet 0017"`
