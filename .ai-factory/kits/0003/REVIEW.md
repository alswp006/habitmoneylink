당신은 패킷 0003 ("엔티티 get/set + 최소 스키마 검증(타입가드)")의 코드 리뷰를 수행합니다.

## 리뷰 대상 파일
src/lib/storage/validate.ts
src/lib/storage/entities.ts

## 이 패킷의 승인 기준 (AC)
(spec.md 참조)

## 테스트 파일
src/__tests__/packet-0003.test.ts
...
- src/lib/storage/validate.ts
- src/lib/storage/entities.ts
- src/__tests__/packet-0003.test.ts (존재하는 경우)
...
### Agent 2: 조용한 실패 탐지기
각 파일을 읽으세요: src/lib/storage/validate.ts
src/lib/storage/entities.ts
...
```
════════════════════════════════════════════════
🔍 코드 리뷰 리포트: 0003 — 엔티티 get/set + 최소 스키마 검증(타입가드)
════════════════════════════════════════════════
