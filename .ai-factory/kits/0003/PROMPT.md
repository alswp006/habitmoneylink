## Task: 엔티티 get/set + 최소 스키마 검증(타입가드)
5개 저장소(habits/checkins/goals/badges/settings)에 대해 get*/set*를 구현하고, 파싱 성공 여부와 무관하게 최소 스키마 검증 실패 시 SCHEMA_MISMATCH(500)로 통일해 throw 한다. goals는 active가 2개 이상이면 즉시 스키마 불일치로 처리한다.
...
src/lib/storage/validate.ts, src/lib/storage/entities.ts
...
- Packet 0002: "localStorage base IO 유틸 + 키 상수" (files: src/lib/storage/keys.ts, src/lib/storage/base.ts)
