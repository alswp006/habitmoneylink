## Task: S3 습관 수정 /habit/:habitId/edit (조회/수정/삭제/활성 토글)
습관 수정 화면을 최소 기능으로 구현한다. habitId로 기존 습관을 찾아 제목/금액을 수정하고, isActive는 Switch로 토글한다. 삭제 버튼은 AlertDialog로 확인 후 deleteHabit을 호출한다. habitId가 없으면 NOT_FOUND(404) 상태로 안내 문구만 보여준다.
