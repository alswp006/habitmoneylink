## Task: Integration 2/2: 광고/보상형 게이트/TabBar/507 토스트 폴리시 최종 폴리시
App.tsx에 하단 TabBar(홈/리포트/배지)를 플로팅 고정으로 배치하고 safe-area padding을 적용한다. '/report'와 '/badge/unlock' 라우트는 TossRewardAd로 감싸 결과 노출을 게이트한다. 또한 QUOTA_EXCEEDED(507) 발생 시 앱 크래시 대신 전역 Toast로 안내하는 폴리시를 추가한다.
...
## Page Type: Result/Output (TDS 패턴)
참조: tds-patterns.md의 "StatusPanel" + "ButtonStack". AI 결과면 "AI가 생성한" 라벨 의무.
```tsx
<>
  ...
  <div style={{ padding: '16px 24px' }}>
    <div style={{ padding: 20, borderRadius: 16, backgroundColor: 'var(--adaptiveLayeredBackground)' }}>
      <Paragraph.Text typography="st11" color="secondary">오늘의 결과</Paragraph.Text>
      <Spacing size={4} />
      <Paragraph.Text typography="t1">{value}</Paragraph.Text>
      {isAi && (<><Spacing size={8} /><Paragraph.Text typography="st13" color="tertiary">AI가 생성한 결과예요</Paragraph.Text></>)}
    </div>
    <Spacing size={24} />
    <ListRow contents={<ListRow.Texts type="Right2RowTypeA" top="라벨" bottom="설명" />} />
  </div>
  <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, display: 'flex', flexDirection: 'column', gap: 8, padding: 'calc(var(--toss-safe-area-bottom) + 12px) 16px 12px' }}>
    <Button variant="fill" onClick={handlePrimary}>다시 시도</Button>
    <Button variant="weak" onClick={() => navigate('/')}>처음으로</Button>
  </div>
</>
```
...
- Packet 0015: "Integration 1/2: App.tsx 라우팅 연결 + 세션/실행환경 가드(401)" (files: src/App.tsx)
...
Create `src/__tests__/packet-0016.test.ts` with tests for the Acceptance Criteria above.
...
- Commit: `git add src/__tests__/packet-0016.test.ts && git commit -m "test: TDD red phase — packet 0016"`

### Step 2: Verify Red (tests should FAIL)
```bash
pnpm test src/__tests__/packet-0016.test.ts
```
...
- Commit: `git add <source files> && git commit -m "feat: implement packet 0016"`
