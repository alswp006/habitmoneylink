## Task: Integration 1/2: App.tsx 라우팅 연결 + 세션/실행환경 가드(401)
react-router-dom 라우팅을 App.tsx에 연결하고, 앱 시작 시 getIsTossLoginIntegratedService()를 try/catch로 1회 호출해 예외가 나면 어떤 경로로 들어와도 Blocking 화면만 렌더링하는 전역 가드를 적용한다. 이 패킷에서는 광고/게이트/폴리시는 다루지 않는다.
...
- Packet 0007: "S0 차단 화면 Blocking" (files: src/pages/Blocking.tsx)
- Packet 0008: "S1 홈 / (로딩/빈상태/에러다이얼로그/체크인/요약/배너)" (files: src/pages/Home.tsx)
- Packet 0009: "S2 습관 생성 /habit/new (카테고리 시트 + 입력 + 저장)" (files: src/pages/HabitNew.tsx)
- Packet 0010: "S3 습관 수정 /habit/:habitId/edit (조회/수정/삭제/활성 토글)" (files: src/pages/HabitEdit.tsx)
- Packet 0011: "S4 목표 설정 /goal (활성 목표 upsert)" (files: src/pages/Goal.tsx)
- Packet 0012: "S5 주간 리포트 /report (요약 화면, 게이트는 라우팅에서 적용)" (files: src/pages/Report.tsx)
- Packet 0013: "S6 배지 보관함 /badge (배지 리스트)" (files: src/pages/Badge.tsx)
- Packet 0014: "S7 배지 잠금 해제 /badge/unlock (언락 요청, 중복 409 처리)" (files: src/pages/BadgeUnlock.tsx)
