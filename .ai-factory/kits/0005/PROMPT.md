## Task: 공용 UI 컴포넌트: 요약 카드(HeroSummaryCard)
홈 상단의 ‘누적 절약액/목표 요약’을 반복 가능한 단일 컴포넌트로 분리한다. TDS Typography/Paragraph.Text/Button/Spacing만 사용하고, 레이아웃을 위한 최소한의 div 스타일만 허용한다. 숫자 포맷(원 단위)은 호출자가 문자열로 전달한다.
...
src/components/HeroSummaryCard.tsx
...
## Page Type: Home (TDS 패턴)
참조: 골든 템플릿 `src/pages/Home.tsx` (이미 작성됨). Top + 본문 + FixedBottomCTA 조합.
새 Home은 기존 골든을 베이스로 변형하세요. 추가 섹션은 ListRow stack 또는 StatusPanel 패턴 활용.
